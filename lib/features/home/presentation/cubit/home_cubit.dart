...
class HomeCubit extends Cubit<HomeState> {
  ...
  final BreedSearchUseCases breedSearchUseCases;
  HomeCubit({required this.breedUseCases, required this.breedSearchUseCases})
    : super(HomeInitial());
  ...
  void getDataSearchState({
    required String search,
    required int attchImage,
  }) async {
    emit(HomeDataSearchLoading());

    final response = await breedSearchUseCases.getDataSearch(
    ...
    if (response is Success<List<Breed>>) {
      emit(HomeDataSearchSuccess(dataSearch: response.data ?? []));
    } else if (response is Failure<List<Breed>>) {
      emit(
        HomeFailure(
          errorMessage: ApiErrorModel(
